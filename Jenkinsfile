node {
    ...
    dir('temp'){
        ...
        withEnv(["GOROOT=$goRoot","GOPATH=${cliWorkspace}","PATH+GOROOT=${goRoot}/bin", "JFROG_CLI_OFFER_CONFIG=false"]) {
            stage 'Go get'
            sh 'go get -f -u github.com/AlexeiVainshtein/jfrog-cli-go-modules/jfrog-cli/jfrog'
            if (BRANCH?.trim()) {
                dir("src/github.com/AlexeiVainshtein/jfrog-cli-go-modules/jfrog-cli/jfrog") {
...
def buildAndUpload(goos, goarch, pkg, fileExtension) {
    sh "env GOOS=$goos GOARCH=$goarch go build github.com/AlexeiVainshtein/jfrog-cli-go-modules/jfrog-cli/jfrog"
...
def publishNpmPackage() {
    dir ('src/github.com/AlexeiVainshtein/jfrog-cli-gonpm/') {
        sh '''#!/bin/bash
